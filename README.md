...
- `read_console`, `inspect_network_requests`, and `inspect_event_listeners` provide bounded diagnostics on Chrome. Capture starts before either streaming or non-streaming Dev runs and stops when the tab leaves Dev mode or its conversation is cleared; WebBrain removes its handlers and buffers and disables the matching CDP domains. Network headers and bodies are omitted by default, sensitive header names are redacted before buffering, and page-derived diagnostic output is treated as untrusted content.
