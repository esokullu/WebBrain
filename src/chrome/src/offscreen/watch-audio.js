/**
 * Audio host for /watch alerts and background-owned completion chimes. The
 * scheduler persists the event key before asking this offscreen document to
 * play, so restarts cannot double-play the same event. These generated tones
 * are intentionally distinct from one another.
 */
...
(() => {
  ...
  function completionPattern(success) {
    return success
      ? [
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.target !== 'offscreen-watch-audio') return;
    if (message.action === 'play_watch_alert') {
    ...
    if (message.action === 'play_completion_chime') {
      playback = playback.catch(() => {}).then(() => playPattern(completionPattern(message.success !== false)));
      playback.then(
        () => sendResponse({ ok: true }),
        (error) => sendResponse({ ok: false, error: String(error?.message || error) }),
      );
